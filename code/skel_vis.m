...
skel_model
skel=reshape(xyz_ti, 3, NUI_SKELETON_POSITION_COUNT)';
if nargin < 3
	h=axes;
end
...
for ci=1:size(nui_skeleton_conn,1)
	hold on;
	line([xyz_ti(3*nui_skeleton_conn(ci,1)+1) ; ...
		xyz_ti(3*nui_skeleton_conn(ci,2)+1)], ...
		[xyz_ti(3*nui_skeleton_conn(ci,1)+2) ; ...
		xyz_ti(3*nui_skeleton_conn(ci,2)+2)], ...
		[xyz_ti(3*nui_skeleton_conn(ci,1)+3) ; ...
		xyz_ti(3*nui_skeleton_conn(ci,2)+3)]);
end
...
xmin=min(min(X(:,1:3:size(X,2))));
ymin=min(min(X(:,2:3:size(X,2))));
zmin=min(min(X(:,3:3:size(X,2))));
xmax=max(max(X(:,1:3:size(X,2))));
ymax=max(max(X(:,2:3:size(X,2))));
zmax=max(max(X(:,3:3:size(X,2))));
